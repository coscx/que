...
import 'package:flutter_geen/views/component/pop_menu.dart';
              ...
              return GestureDetector(
                  onTap: () async {
                    ...
                    final Conversion model = Conversion.fromMap({
                      "memId": memberId,
                      "cid": state.message[index].cid,
                      "name": state.message[index].name
                    });
                    if (state.message[index].type ==
                        ConversionType.CONVERSATION_GROUP) {
                      BlocProvider.of<GroupBloc>(context).add(
                          EventGroupFirstLoadMessage(
                              memberId, state.message[index].cid));
                      Navigator.pushNamed(context, UnitRouter.to_group_chat,
                          arguments: model);
                    }
                    if (state.message[index].type ==
                        ConversionType.CONVERSATION_PEER) {
                      BlocProvider.of<PeerBloc>(context).add(
                          EventFirstLoadMessage(
                              memberId, state.message[index].cid));
                      Navigator.pushNamed(context, UnitRouter.to_chats,
                          arguments: model);
                    }
                  },
